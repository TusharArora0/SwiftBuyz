...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch stats
      const statsResponse = await fetch('https://swiftbuyz-five.vercel.app/api/admin/stats', {
        headers
      });
      if (!statsResponse.ok) {
      ...
      // Fetch all users
      const usersResponse = await fetch('https://swiftbuyz-five.vercel.app/api/admin/users', {
        headers
      });
      if (!usersResponse.ok) {
      ...
      // Fetch all sellers
      const sellersResponse = await fetch('https://swiftbuyz-five.vercel.app/api/admin/sellers', {
        headers
      });
      ...
      // Fetch all orders
      const ordersResponse = await fetch('https://swiftbuyz-five.vercel.app/api/admin/orders', {
        headers
      });
  ...
  const handleUserStatusChange = async (userId, isBlocked) => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`https://swiftbuyz-five.vercel.app/api/admin/users/${userId}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleSellerApproval = async (sellerId, isApproved) => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`https://swiftbuyz-five.vercel.app/api/admin/sellers/${sellerId}/approve`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteConfirm = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`https://swiftbuyz-five.vercel.app/api/admin/users/${userToDelete._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
